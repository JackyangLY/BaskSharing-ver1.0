...
-(void)loadBasicSetting
{
    ...
    else
    {
        /**打印此方法*/
//        NSLog(@"%s\n邮箱错误",__func__);
        [self showAlterControllerWithString:nil titleWithMessage:@"邮箱错误"];
    }
...
//            NSLog(@"%s  验证码发送成功succeeded---%@",__func__,error);
            /**弹框提示*/
            [self showAlterControllerWithString:@"邮箱重置密码提示:"  titleWithMessage:@"验证码发送成功"];
...
//             NSLog(@"%s  用户名未注册---%@",__func__,error);
            /**弹框提示*/
            [self showAlterControllerWithString:@"邮箱重置密码提示:" titleWithMessage:@"用户名不存在或错误" ];
        }
    }];
